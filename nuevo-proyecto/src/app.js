...
// URLs integradas de las hojas de Google Sheets (compartidas como "cualquiera con el enlace").
// Método gviz (compatible con hojas compartidas, no requiere "Publicar en la web").
const BUILTIN_CONFIG = {
  catalogUrl: 'https://docs.google.com/spreadsheets/d/1UDY7vse-NqjQcBYSgsSdS3bT7s-MiZl_w_uaTcCOyUo/gviz/tq?tqx=out:csv&gid=1621024041',
  usersUrl: 'https://docs.google.com/spreadsheets/d/1dFBcAjUso7VFDn1KYapRI7gE826NIWWKRZL4o0OT5E0/gviz/tq?tqx=out:csv&gid=435631673',
  adminPass: 'gemeseg2026'  // clave para abrir "Configuración de administrador" (cámbiala si quieres)
};

function getAuthConfig() {
  const stored = JSON.parse(localStorage.getItem('auth_config') || '{}');
  return {
    catalogUrl: BUILTIN_CONFIG.catalogUrl || stored.catalogUrl || '',
    usersUrl: BUILTIN_CONFIG.usersUrl || stored.usersUrl || '',
  };
}
...
async function fetchUsers(url) {
  const text = await fetchCsv(url);
  const rows = parseCSV(text);
  if (rows.length < 2) throw new Error('Hoja de usuarios vacía');
  const header = rows[0].map(h => (h||'').trim().toLowerCase());
  const ci = {
    name: header.findIndex(h => h.includes('nombre')),
    user: header.findIndex(h => h.includes('usuario') || h.includes('user')),
    pass: header.findIndex(h => h.includes('clave') || h.includes('pass') || h.includes('contras')),
    active: header.findIndex(h => h.includes('activo') || h.includes('active') || h.includes('estado')),
  };
  if (ci.user === -1 || ci.pass === -1) throw new Error('La hoja debe tener columnas Usuario y Clave');
  const users = [];
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r[ci.user] || !r[ci.user].trim()) continue;
    const activeVal = (ci.active !== -1 ? (r[ci.active]||'') : 'si').trim().toLowerCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    users.push({
      name: ci.name !== -1 ? (r[ci.name]||'').trim() : r[ci.user].trim(),
      user: r[ci.user].trim(),
      pass: (r[ci.pass]||'').trim(),
      active: /^(si|yes|true|1|activo|x)$/.test(activeVal),
    });
  }
  return users;
}
...
async function doLogin() {
  ...
  if (!user || !pass) { showLoginError('Ingresa usuario y clave'); return; }
  const cfg = getAuthConfig();
  if (!cfg.usersUrl) { showLoginError('La app aún no está configurada. Pulsa "Configuración de administrador" abajo.'); return; }
  ...
  try {
    const users = await fetchUsers(cfg.usersUrl);
    const match = users.find(u => u.user.toLowerCase() === user.toLowerCase() && u.pass === pass);
    if (!match) { showLoginError('Usuario o clave incorrectos.'); resetLoginBtn(); return; }
    if (!match.active) { showLoginError('Tu acceso fue desactivado. Contacta al administrador.'); resetLoginBtn(); return; }
    const session = { user: match.user, name: match.name, ts: Date.now() };
    localStorage.setItem('session', JSON.stringify(session));
    enterApp(session);
  } catch(e) {
    const isLocal = location.protocol === 'file:';
    showLoginError(isLocal
      ? 'No se puede verificar desde un archivo local. La app debe abrirse desde su dirección web (hospedada).'
      : 'No se pudo verificar el acceso. Revisa tu conexión. (' + e.message + ')');
    resetLoginBtn();
  }
}
...
function enterApp(session) {
  ...
  $('userChipName').textContent = session.name || session.user;
  resetLoginBtn();
  // Arrancar la app
  bootApp();
}
...
// Revalida la sesión guardada contra el Sheet (detecta revocaciones)
async function validateSession() {
  ...
  const cfg = getAuthConfig();
  if (!cfg.usersUrl) return false;
  // Si no hay internet o falla, permitir entrar con sesión previa (modo tolerante)
  // pero si responde y el usuario está inactivo, bloquear.
  try {
    const users = await fetchUsers(cfg.usersUrl);
    const match = users.find(u => u.user.toLowerCase() === session.user.toLowerCase());
    ...
    // Sin conexión: confiar en sesión local si es reciente (< 7 días)
...
function openConfigSetup() {
  $('loginNormal').style.display = 'none';
  $('loginConfig').style.display = 'block';
  const cfg = getAuthConfig();
  $('setupCatalogUrl').value = cfg.catalogUrl || '';
  $('setupUsersUrl').value = cfg.usersUrl || '';
}
function closeConfigSetup() {
  $('loginConfig').style.display = 'none';
  $('loginNormal').style.display = 'block';
}
function saveSetup() {
  const adminPass = $('adminPass').value.trim();
  if (adminPass !== BUILTIN_CONFIG.adminPass) {
    $('configError').textContent = 'Clave de administrador incorrecta';
    $('configError').classList.add('show');
    return;
  }
  const catalogUrl = $('setupCatalogUrl').value.trim();
  const usersUrl = $('setupUsersUrl').value.trim();
  if (!usersUrl) {
    $('configError').textContent = 'La URL de USUARIOS es obligatoria';
    $('configError').classList.add('show');
    return;
  }
  const authCfg = { catalogUrl, usersUrl };
  localStorage.setItem('auth_config', JSON.stringify(authCfg));
  // También guardar la URL del catálogo en la config de sync
  if (catalogUrl) {
    const c = getConfig();
    c.driveUrl = catalogUrl;
    setConfig(c);
  }
...
function bootApp() {
  // Conectar URL de catálogo desde la config de auth (para que todos los dispositivos sincronicen)
  const authCfg = getAuthConfig();
  if (authCfg.catalogUrl) {
    const c = getConfig();
    if (c.driveUrl !== authCfg.catalogUrl) {
      c.driveUrl = authCfg.catalogUrl;
      setConfig(c);
    }
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ¿Hay sesión válida? → entrar directo
