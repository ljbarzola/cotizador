...
function buildEquipo(mapped) {
  return {
    ...
    costo_total: num(mapped.costo_total) || 0,
...
async function getTableColumns(tableName) {
  ...
  if (!insErr) {
    const { data: rowData } = await supabase
      .from(tableName)
      .select('*')
      .eq('source_id', '___schema_probe___')
      .maybeSingle();
...
const COLUMN_LABELS = {
  ...
  producto: 'Producto',
  unidades: 'Unds',
  cantidad_default: 'Cant.',
  costo_unitario: 'Costo Unit.',
  costo_total: 'Costo',
  proveedor: 'Proveedor',
  observaciones: 'Observaciones',
  ultima_act: 'Última act.',
  servicio: 'Servicio',
  descripcion: 'Descripción',
  costo_mensual: 'Costo mensual',
  costo_anual: 'Costo anual',
  ganancia_flag: 'Gan. Prov.',
  instalacion_flag: 'Gan. Inst.',
  source_id: 'Código',
};
...
export async function syncFromGoogleSheets(onProgress, signal) {
  ...
  for (const sheet of SHEETS) {
    ...
    try {
      ...
      const { data: existingRows, error: fetchErr } = await supabase.from(sheet.table).select('id, source_id');
  ...
  if (aborted) {
    ...
    log(
      `RESUMEN: +${results.inserted} nuevos ~${results.updated} actualizados =${results.unchanged} sin cambio ✕${results.failed} errores`
    );
  }
  return results;
}
